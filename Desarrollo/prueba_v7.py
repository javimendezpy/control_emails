...
# Opción A: automáticamente ayer
# fecha_referencia = (pd.Timestamp.now()-pd.Timedelta(days=1)).date() # Formato

# Opción B: pasar una fecha manual (ejemplo: 2025-09-08)
fecha_referencia = pd.Timestamp(2025, 9, 8).date()
...
fecha_ini = pd.Timestamp(fecha_referencia).strftime('%m/%d/%Y 00:00 %p')
fecha_fin = pd.Timestamp(fecha_referencia + pd.Timedelta(days=1)).strftime('%m/%d/%Y 11:59 PM')
...
# Si la fecha ya existe en las columnas -> reescribir
