...
# fecha_actual = pd.Timestamp.now()-pd.Timedelta(days=1) # Formato


# fecha_objetivo = (fecha_actual - pd.Timedelta(days=1)).date()
fecha_objetivo = pd.Timestamp(2025,8,25,0,0,0).date()  # ejemplo, puedes cambiar a la fecha que te interese ; busca los datos del dia 11/08/2025
print('Fecha objetivo: ', fecha_objetivo)

print('Esperamos encontrar los archivos de datos del día ',fecha_objetivo, ' en el día ', fecha_objetivo+pd.Timedelta(days=1))
